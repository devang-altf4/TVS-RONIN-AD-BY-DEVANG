        ...
        <section className="py-24 bg-[#080808] text-white relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                >
                    ...
                    <p className="text-gray-400 text-lg tracking-widest uppercase">Select your defining style</p>
                </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {variants.map((variant, index) => (
                        <motion.div 
                            ...
                            className="bg-[#111] border border-gray-800 p-8 hover:border-red-600 transition-colors duration-300 group relative"
                        >
                            <div className={`absolute top-0 left-0 w-full h-1 ${variant.color} opacity-50 group-hover:opacity-100 transition-opacity`}></div>
                            <h3 className="text-2xl font-bold uppercase mb-2">{variant.name}</h3>
                            <p className="text-3xl font-black text-red-500 mb-6">{variant.price}</p>
                            
                            <ul className="mb-8 space-y-3">
                                {variant.features.map((feature, i) => (
                                    <li key={i} className="flex items-center text-gray-400 text-sm">
                                        <div className="w-1.5 h-1.5 bg-red-600 mr-3 rounded-full"></div>
                                        {feature}
                                    </li>
                                ))}
                            </ul>

                            <button className="w-full py-4 border border-gray-600 text-white uppercase tracking-widest hover:bg-white hover:text-black transition-all hover:font-bold">
                                View Details
                            </button>
                        </motion.div>
                    ))}
                </div>
                <p className="text-center text-xs text-gray-600 mt-12">*Ex-showroom price. Prices may vary by state.</p>
            </div>
        </section>
    );
};
